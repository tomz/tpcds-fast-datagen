...
### Distributed — `spark_tpcds_gen.py`

Self-contained script (no dependency on the `tpcds_fast_datagen` package; Python 3.6 driver compatible). Plans the same chunked task list, parallelises via `mapPartitions` across YARN executors. Each executor task uses the same `.dat` → PyArrow → Parquet streaming pipeline. Output goes to HDFS / ABFS / GCS / S3 via `fs.defaultFS`.
